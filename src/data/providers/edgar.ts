...
import type { FilingMetadata, FinancialMetrics } from "../../agents/types.js";
...
export class EdgarDataProvider {
  ...
  async getCik(symbol: string): Promise<string> {
    ...
    const response = await rateLimitedFetch(`${EDGAR_BASE_URL}/submissions/company_tickers.json`);
  ...
  async getFilings(
    ...
    const response = await rateLimitedFetch(`${EDGAR_BASE_URL}/submissions/CIK${cik}.json`);
  ...
  async getFinancialFacts(symbol: string): Promise<FinancialMetrics | null> {
    ...
    const response = await rateLimitedFetch(`${EDGAR_BASE_URL}/api/xbrl/companyfacts/CIK${cik}.json`);
    ...
    const getLatestAnnual = (concept: string): number | undefined => {
      ...
      const annual = units.filter((u) => u.form === "10-K" && u.fp === "FY").sort((a, b) => b.fy - a.fy);

      return annual[0]?.val;
    };

    const _getLatestQuarterly = (concept: string): number | undefined => {
      const fact = gaap[concept];
      if (!fact) return undefined;
    ...
    const _totalAssets = getLatestAnnual("Assets");
    ...
    const debtToEquity = totalLiabilities && stockholdersEquity ? totalLiabilities / stockholdersEquity : undefined;
    const currentRatio = currentAssets && currentLiabilities ? currentAssets / currentLiabilities : undefined;
    const freeCashFlow = operatingCashFlow && capex ? operatingCashFlow - capex : undefined;
    const roe = netIncome && stockholdersEquity ? netIncome / stockholdersEquity : undefined;
